...
# In this hcl file, I added the implementation of the IIADDQ instruction
# This instruction performs an add that adds an immediate value to a register
# IIADDQ had to be added for condition code checking in execute stage

#The following changes in the instruction sequence were required for IADDQ

#Fetch Stage:
	#Add the instruction in the icode section. Give it an instruction code
	#set the instruction to need register id's
	#Set the instruction to require val_C. This is the immediate value to add
#Decode Stage:
	#Set source B. This is the register that acts as the destination
	#Set dstE to the destination register, rB
#Execute:
	#Set ALU to use val_C (immediate)
	#Set the ALU to use the destination register, val_B
	#Set the condition codes for the instruction	
